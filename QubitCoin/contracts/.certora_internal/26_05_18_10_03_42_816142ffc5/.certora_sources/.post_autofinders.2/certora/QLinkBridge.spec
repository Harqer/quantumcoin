// QLinkBridge Certora Specification - Fixed with Latest CVL Syntax
...
methods {
    ...
    // State-changing functions - use NONDET summary for soundness
    // Per docs: NONDET is sound for view functions, HAVOC_ALL for state-changing
    function _.initiateTransfer(address, uint256, address, uint256) external returns (bytes32) => NONDET;
    function _.submitProof(bytes32, bytes, bytes32) external => HAVOC_ALL;
    function _.executeTransfer(bytes32) external => HAVOC_ALL;
    function _.addValidator(address) external => HAVOC_ALL;
    function _.supportToken(address) external => HAVOC_ALL;
    function _.supportChain(uint256) external => HAVOC_ALL;
}
...
// RULE-10: Proof threshold integrity
