      ...
      try {
            ...
            setOrderBook((prev) => {
              ...
              updates.forEach(
                (u: {
                  ...
                  if (u.side === "bid")
                    newBids.push([u.price_level, u.new_quantity]);
                  if (u.side === "offer")
                    newAsks.push([u.price_level, u.new_quantity]);
                },
              );

              newBids = newBids
                .filter((b) => parseFloat(b[1]) > 0)
                .sort((a, b) => parseFloat(b[0]) - parseFloat(a[0]))
                .slice(0, 10);
              newAsks = newAsks
                .filter((a) => parseFloat(a[1]) > 0)
        ...
        if (data.channel === "market_trades" && data.events) {
          ...
          if (Array.isArray(tradesList)) {
            const newTrades = tradesList.map(
              (t: { time: string; price: string }) => ({
  ...
  useEffect(() => {
    if (seriesRef.current && trades.length > 0) {
      ...
      const uniqueTrades = Array.from(
        new Map(trades.map((t) => [t.time, t])).values(),
      );
  ...
  return (
    <div className="min-h-screen bg-[#0A0B0D] text-white font-sans">
      <nav className="flex justify-between items-center px-6 py-4 bg-[#111214] border-b border-[#2B2F36]">
        <div className="text-xl font-bold tracking-widest text-[#0052ff]">
          QUANTUM TRADE
        </div>
        <div>
          {isMounted && (
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-[#111214] p-4 rounded-lg border border-[#2B2F36] flex justify-between items-center">
            <div className="flex items-center gap-8">
              ...
              >
                <option value="BTC-USD" className="bg-[#111214] text-base">
                  BTC-USD
                </option>
                <option value="ETH-USD" className="bg-[#111214] text-base">
                  ETH-USD
                </option>
                <option value="SOL-USD" className="bg-[#111214] text-base">
                  SOL-USD
                </option>
              </select>
              <div>
                ...
                <p className="text-2xl font-medium text-[#12B981]">
                  ${ticker?.price || "..."}
                </p>
              </div>
              <div>
                ...
                <p className="text-xl font-medium text-gray-200">
                  {ticker?.volume_24h
                    ? parseFloat(ticker.volume_24h).toFixed(2)
                    : "..."}{" "}
                  BTC
                </p>
              </div>
                ...
                <div className="space-y-1 font-mono text-sm">
                  {orderBook.bids.length > 0 ? (
                    orderBook.bids.map((bid, i) => (
                      <div
                        key={`bid-${i}`}
                        className="flex justify-between px-2 text-[#12B981] hover:bg-[#12B981]/10 py-1 rounded cursor-pointer transition-colors"
                      >
                ...
                <div className="space-y-1 font-mono text-sm">
                  {orderBook.asks.length > 0 ? (
                    orderBook.asks.map((ask, i) => (
                      <div
                        key={`ask-${i}`}
                        className="flex justify-between px-2 text-[#ff5b5a] hover:bg-[#ff5b5a]/10 py-1 rounded cursor-pointer transition-colors"
                      >
