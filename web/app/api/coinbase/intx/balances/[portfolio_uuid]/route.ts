...
export async function GET(req: Request, { params }: { params: Promise<{ portfolio_uuid: string }> }) {
  try {
    ...
    const response = await fetch(`https://api.coinbase.com/api/v3/brokerage/intx/balances/${resolvedParams.portfolio_uuid}`, {
      headers: {
    ...
    console.error('Error fetching intx balances:', error);
    return NextResponse.json({ error: 'Failed to fetch intx balances' }, { status: 500 });
  }
}
