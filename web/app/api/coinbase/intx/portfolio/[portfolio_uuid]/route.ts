...
export async function GET(req: Request, { params }: { params: Promise<{ portfolio_uuid: string }> }) {
  try {
    ...
    const response = await fetch(`https://api.coinbase.com/api/v3/brokerage/intx/portfolio/${resolvedParams.portfolio_uuid}`, {
      headers: {
    ...
    console.error('Error fetching intx portfolio:', error);
    return NextResponse.json({ error: 'Failed to fetch intx portfolio' }, { status: 500 });
  }
}
