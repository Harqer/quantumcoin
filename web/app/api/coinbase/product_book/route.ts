...
export async function GET(req: Request) {
  try {
    ...
    const aggregation_price_increment = searchParams.get(
    ...
    if (aggregation_price_increment)
      params.append("aggregation_price_increment", aggregation_price_increment);

    const url = `https://api.coinbase.com/api/v3/brokerage/product_book?${params.toString()}`;
    ...
    return NextResponse.json(
      { error: "Failed to fetch product book" },
      { status: 500 },
    );
  }
}
