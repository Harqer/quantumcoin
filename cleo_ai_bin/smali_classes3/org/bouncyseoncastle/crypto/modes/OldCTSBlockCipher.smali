.class public Lorg/bouncyseoncastle/crypto/modes/OldCTSBlockCipher;
.super Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;
.source "SourceFile"


# virtual methods
.method public a(I)I
    .locals 0

    .line 2
    iget p0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    add-int/2addr p1, p0

    return p1
.end method

.method public a([BI)I
    .locals 9

    .line 1
    iget v0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    add-int/2addr v0, p2

    array-length v1, p1

    if-gt v0, v1, :cond_7

    iget-object v0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    invoke-interface {v0}, Lorg/bouncyseoncastle/crypto/BlockCipher;->d()I

    move-result v0

    iget v1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    sub-int/2addr v1, v0

    new-array v2, v0, [B

    iget-boolean v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->j:Z

    const/4 v4, 0x0

    if-eqz v3, :cond_4

    iget-object v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v3, v5, v4, v2, v4}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    iget v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    if-lt v3, v0, :cond_3

    :goto_0
    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    array-length v6, v5

    if-eq v3, v6, :cond_0

    sub-int v6, v3, v0

    aget-byte v6, v2, v6

    aput-byte v6, v5, v3

    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    :cond_0
    move v3, v0

    :goto_1
    iget v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    if-eq v3, v5, :cond_1

    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    aget-byte v6, v5, v3

    sub-int v7, v3, v0

    aget-byte v7, v2, v7

    xor-int/2addr v6, v7

    int-to-byte v6, v6

    aput-byte v6, v5, v3

    add-int/lit8 v3, v3, 0x1

    goto :goto_1

    :cond_1
    iget-object v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    instance-of v5, v3, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;

    if-eqz v5, :cond_2

    check-cast v3, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;

    invoke-virtual {v3}, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;->c()Lorg/bouncyseoncastle/crypto/BlockCipher;

    move-result-object v3

    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v3, v5, v0, p1, p2}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    goto :goto_2

    :cond_2
    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v3, v5, v0, p1, p2}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    :goto_2
    add-int/2addr p2, v0

    invoke-static {v2, v4, p1, p2, v1}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    goto :goto_5

    :cond_3
    new-instance p0, Lorg/bouncyseoncastle/crypto/DataLengthException;

    const-string p1, "need at least one block of input for CTS"

    invoke-direct {p0, p1}, Lorg/bouncyseoncastle/crypto/DataLengthException;-><init>(Ljava/lang/String;)V

    throw p0

    :cond_4
    new-array v3, v0, [B

    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    instance-of v6, v5, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;

    if-eqz v6, :cond_5

    check-cast v5, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;

    invoke-virtual {v5}, Lorg/bouncyseoncastle/crypto/modes/CBCBlockCipher;->c()Lorg/bouncyseoncastle/crypto/BlockCipher;

    move-result-object v5

    iget-object v6, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v5, v6, v4, v2, v4}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    goto :goto_3

    :cond_5
    iget-object v6, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v5, v6, v4, v2, v4}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    :goto_3
    move v5, v0

    :goto_4
    iget v6, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    if-eq v5, v6, :cond_6

    sub-int v6, v5, v0

    aget-byte v7, v2, v6

    iget-object v8, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    aget-byte v8, v8, v5

    xor-int/2addr v7, v8

    int-to-byte v7, v7

    aput-byte v7, v3, v6

    add-int/lit8 v5, v5, 0x1

    goto :goto_4

    :cond_6
    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-static {v5, v0, v2, v4, v1}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget-object v5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    invoke-interface {v5, v2, v4, p1, p2}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    add-int/2addr p2, v0

    invoke-static {v3, v4, p1, p2, v1}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    :goto_5
    iget p1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    invoke-virtual {p0}, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->c()V

    return p1

    :cond_7
    new-instance p0, Lorg/bouncyseoncastle/crypto/OutputLengthException;

    const-string p1, "output buffer to small in doFinal"

    invoke-direct {p0, p1}, Lorg/bouncyseoncastle/crypto/OutputLengthException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public a([BII[BI)I
    .locals 6

    if-ltz p3, :cond_4

    .line 3
    invoke-virtual {p0}, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->a()I

    move-result v0

    invoke-virtual {p0, p3}, Lorg/bouncyseoncastle/crypto/modes/OldCTSBlockCipher;->b(I)I

    move-result v1

    if-lez v1, :cond_1

    add-int/2addr v1, p5

    array-length v2, p4

    if-gt v1, v2, :cond_0

    goto :goto_0

    :cond_0
    new-instance p0, Lorg/bouncyseoncastle/crypto/OutputLengthException;

    const-string p1, "output buffer too short"

    invoke-direct {p0, p1}, Lorg/bouncyseoncastle/crypto/OutputLengthException;-><init>(Ljava/lang/String;)V

    throw p0

    :cond_1
    :goto_0
    iget-object v1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    array-length v2, v1

    iget v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    sub-int/2addr v2, v3

    const/4 v4, 0x0

    if-le p3, v2, :cond_3

    invoke-static {p1, p2, v1, v3, v2}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget-object v1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    iget-object v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-interface {v1, v3, v4, p4, p5}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    move-result v1

    iget-object v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-static {v3, v0, v3, v4, v0}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iput v0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    sub-int/2addr p3, v2

    add-int/2addr p2, v2

    :goto_1
    if-le p3, v0, :cond_2

    iget-object v2, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    iget v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    invoke-static {p1, p2, v2, v3, v0}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget-object v2, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->k:Lorg/bouncyseoncastle/crypto/BlockCipher;

    iget-object v3, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    add-int v5, p5, v1

    invoke-interface {v2, v3, v4, p4, v5}, Lorg/bouncyseoncastle/crypto/BlockCipher;->a([BI[BI)I

    move-result v2

    add-int/2addr v1, v2

    iget-object v2, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    invoke-static {v2, v0, v2, v4, v0}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    sub-int/2addr p3, v0

    add-int/2addr p2, v0

    goto :goto_1

    :cond_2
    move v4, v1

    :cond_3
    iget-object p4, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    iget p5, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    invoke-static {p1, p2, p4, p5, p3}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget p1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    add-int/2addr p1, p3

    iput p1, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    return v4

    :cond_4
    new-instance p0, Ljava/lang/IllegalArgumentException;

    const-string p1, "Can\'t have a negative input length!"

    invoke-direct {p0, p1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public b(I)I
    .locals 1

    iget v0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->i:I

    add-int/2addr p1, v0

    iget-object p0, p0, Lorg/bouncyseoncastle/crypto/DefaultBufferedBlockCipher;->h:[B

    array-length v0, p0

    rem-int v0, p1, v0

    if-nez v0, :cond_0

    array-length p0, p0

    sub-int/2addr p1, p0

    return p1

    :cond_0
    sub-int/2addr p1, v0

    return p1
.end method
