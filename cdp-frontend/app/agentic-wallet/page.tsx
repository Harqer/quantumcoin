...
export default function AgenticWalletPage() {
  ...
  } = useCreateEvmEip7702Delegation() as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const {
    ...
    data: spendData,
    error: spendError,
  } = useCreateSpendPermission() as any;
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.description}>
        Manage your autonomous agent permissions and upgrade your wallet to a
        Smart Account.
      </p>
      ...
      <div className={styles.section}>
        ...
        <p>
          Upgrades your existing Externally Owned Account (EOA) in-place to
          support batched transactions and gas sponsorship.
        </p>
        <button
        ...
        {delegationStatus === "success" && (
          <p className={styles.success}>
            Upgrade Complete! Your EOA is now a Smart Account.
          </p>
        )}
        {delegationError && (
          <p className={styles.error}>Error: {delegationError.message}</p>
        )}
      </div>
      ...
      <div className={styles.section}>
        ...
        <p>
          Authorize the Quantum Backend Treasury to spend USDC on your behalf
          for automated agentic trading.
        </p>
        ...
        {spendStatus === "success" && spendData && (
          <p className={styles.success}>
            Permission Granted! Transaction Hash: {spendData.transactionHash}
          </p>
        )}
        {spendError && (
          <p className={styles.error}>Error: {spendError.message}</p>
        )}
      </div>
    </div>
  );
