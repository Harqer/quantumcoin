...
export default function SolanaWallet() {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      <h1 style={{ fontSize: '28px', marginBottom: '20px' }}>Solana Standard Wallet</h1>
      
      {!ready || !wallet ? (
        <div style={{ padding: '20px', background: '#f4f5f8', borderRadius: '12px' }}>
          <p>Wallet not ready. Please ensure you are signed in from the Dashboard.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ padding: '20px', background: '#eef0f3', borderRadius: '12px' }}>
            <h2 style={{ fontSize: '20px', marginBottom: '10px' }}>Your CDP Solana Wallet</h2>
            <p><strong>Name:</strong> {wallet.name}</p>
            <p><strong>Address:</strong> {wallet.accounts[0]?.address}</p>
          </div>

          <div style={{ display: 'flex', gap: '20px' }}>
            <div style={{ flex: 1, padding: '20px', border: '1px solid #d0d5dd', borderRadius: '12px' }}>
              <h3 style={{ fontSize: '18px', marginBottom: '10px' }}>Sign Message</h3>
              <button 
              ...
              {signature && (
                <div style={{ marginTop: '15px', padding: '10px', background: '#f0fdf4', color: '#166534', borderRadius: '8px', wordBreak: 'break-all' }}>
            ...
            <div style={{ flex: 1, padding: '20px', border: '1px solid #d0d5dd', borderRadius: '12px' }}>
              ...
              {txSignature && (
                <div style={{ marginTop: '15px', padding: '10px', background: '#f0fdf4', color: '#166534', borderRadius: '8px', wordBreak: 'break-all' }}>
                  <strong>Tx Sent!</strong><br />{txSignature.slice(0, 30)}...
                </div>
              )}
            </div>
          </div>
      ...
      <div style={{ marginTop: '40px' }}>
        <h3 style={{ fontSize: '20px', marginBottom: '15px' }}>Discovered Standard Wallets ({wallets.length})</h3>
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
          {wallets.map((w, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 20px', border: '1px solid #e0e0e0', borderRadius: '8px' }}>
              {w.icon && <img src={w.icon} alt={w.name} width="24" height="24" />}
              <span>{w.name} {w.features['cdp:'] ? ' (CDP)' : ''}</span>
            </div>
          ))}
        </div>
      </div>
