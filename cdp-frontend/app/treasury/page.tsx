...
export default function TreasuryDashboard() {
  ...
  const fetchTreasuryAccounts = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "https://www.getqubits.com"}/api/treasury/accounts`,
      );
  ...
  const handleCreateAccount = async () => {
    try {
      const name = prompt(
        "Enter a name for the new Custodial Account (e.g., 'Settlement Pool 1'):",
      );
      if (!name) return;
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#666", marginBottom: "30px" }}>
        Entity-owned programmatic wallets for treasury, settlement, and
        operational balances across supported networks.
      </p>
      ...
      {loading ? (
        ...
        <p style={{ color: "red" }}>
          Error: {error}. Ensure the Rust backend is running on port 8001.
        </p>
      ) : accounts.length === 0 ? (
        <div
          style={{
          ...
          {accounts.map((acc, i) => (
            ...
            >
              ...
              <p style={{ fontSize: "14px", color: "#666", margin: "5px 0" }}>
                <strong>ID:</strong> {acc.accountId || acc.id}
              </p>
