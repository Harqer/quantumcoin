...
export const kycRouter = router({
  verifyIdentity: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      let score = 0.98;
      let referenceId = '';
      let docvTransactionToken = null;

      if (!process.env.SOCURE_API_KEY) {
        throw new Error('SOCURE_API_KEY environment variable is not configured.');
      }
  ...
  getStatus: publicProcedure.input(z.object({ userId: z.string() })).query(async ({ input }) => {
