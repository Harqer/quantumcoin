...
export class QuantumBackendOfflineError extends Error {
  constructor(message = 'Quantum backend is offline') {
    super(message);
    this.name = 'QuantumBackendOfflineError';
  }
}
...
const QUANTUM_API_BASE = process.env.NEXT_PUBLIC_QUANTUM_BACKEND_URL || 'http://localhost:8000';
 ...
 */
export const ensureQuantumWalletEncryption = async (userId?: string) => {
  if (!isBrowser()) {
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new QuantumBackendOfflineError();
    }
    ...
    if (userId) {
      await supabase.from('quantum_wallet_backups').upsert({
        user_id: userId,
    ...
    if (error instanceof QuantumBackendOfflineError) {
      throw error;
    }
    throw new QuantumBackendOfflineError('Quantum backend connection failed');
  }
};
...
export const decryptQuantumWallet = async (): Promise<string | null> => {
  ...
  try {
    ...
    throw new QuantumBackendOfflineError('Quantum backend connection failed during decryption');
  }
};
...
export const ensureQuantumSessionEncryption = async (address?: string) => {
  ...
  try {
    ...
    throw new QuantumBackendOfflineError('Quantum backend connection failed during session encryption');
  }
};
...
export const recordEncryptedTransaction = async (
  ...
  try {
    ...
    throw new QuantumBackendOfflineError('Quantum backend connection failed during transaction encryption');
  }
};
