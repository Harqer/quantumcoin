...
export async function GET(req: Request, { params }: { params: Promise<{ product_id: string }> }) {
  try {
    ...
    const get_tradability_status = searchParams.get('get_tradability_status');

    let url = `https://api.coinbase.com/api/v3/brokerage/products/${resolvedParams.product_id}`;
    if (get_tradability_status) {
      url += `?get_tradability_status=${get_tradability_status}`;
    }
