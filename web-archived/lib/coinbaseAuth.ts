 ...
 */
export async function refreshCoinbaseToken(refreshToken: string): Promise<CoinbaseTokenResponse> {
  const clientId = process.env.COINBASE_CLIENT_ID;
  const clientSecret = process.env.COINBASE_CLIENT_SECRET;

  if (!clientId || !clientSecret) {
    throw new Error('Missing COINBASE_CLIENT_ID or COINBASE_CLIENT_SECRET environment variables');
  }
...
export async function fetchWith2FA(
  ...
  if (response.status === 402) {
    try {
      ...
      if (
        errorData.errors && 
        errorData.errors.some((e: any) => e.id === 'two_factor_required')
      ) {
        throw new TwoFactorRequiredError('2FA token required to complete this request.');
      }
    } catch (e) {
    ...
    console.error('Coinbase API: 401 Unauthorized. Access token may be expired or invalid.');
    // Ideally, trigger a token refresh here
  } else if (response.status === 429) {
    console.warn('Coinbase API: 429 Too Many Requests. You are being rate limited.');
  }
...
export async function* fetchPaginatedCoinbaseData<T>(
  ...
  while (currentUrl) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch paginated data: ${response.status} ${errText}`);
    }
