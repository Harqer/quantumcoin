...
export default function SignUpEmailScreen() {
  ...
  }, [email, firstName, lastName, isLoaded, isValidEmail, isRiskEngineReady, deviceSessionId, deviceToken, signUp]);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colorRoles.background.primary }} edges={['top', 'bottom']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1, paddingHorizontal: spacing.l, paddingTop: spacing.xl }}>
          ...
          {!pendingVerification ? (
            <Animated.View key="email-step" entering={FadeInUp.springify().stiffness(80).damping(28)}>
            ...
            <Animated.View key="code-step" entering={FadeInUp.springify().stiffness(80).damping(28)}>
              <Text style={{ fontFamily: typography.titleLarge.fontFamily, fontSize: 32, fontWeight: '700', color: colorRoles.content.primary, marginBottom: spacing.s }}>
                Check your email.
              </Text>
              <Text style={{ fontFamily: typography.bodyLarge.fontFamily, fontSize: 16, color: colorRoles.content.secondary, marginBottom: spacing.xxl, lineHeight: 24 }}>
                We sent a 6-digit code to <Text style={{ fontWeight: '700', color: colorRoles.content.primary }}>{email}</Text>. Next, we'll setup your FaceID/TouchID passkey.
              </Text>
          ...
          {errorMsg ? (
            <Animated.View entering={FadeInDown}>
              <Text style={{ fontFamily: typography.bodyMedium.fontFamily, color: colorRoles.content.negativeDark, marginTop: spacing.m }}>
                {errorMsg}
              </Text>
            </Animated.View>
          ) : null}
        ...
        <Animated.View entering={FadeInDown.springify().stiffness(80).damping(28).delay(300)} style={{ paddingHorizontal: spacing.l, paddingBottom: spacing.xxl }}>
          <PressableScale
            ...
            disabled={(!pendingVerification && !isValidEmail) || (pendingVerification && !isValidCode) || isLoading || !isRiskEngineReady}
            style={{ 
              backgroundColor: ((!pendingVerification && isValidEmail) || (pendingVerification && isValidCode)) ? colorRoles.content.accentMid : colorRoles.background.disabled, 
              paddingVertical: 18, 
              borderRadius: 9999, 
              alignItems: 'center', 
              width: '100%',
          ...
          >
            {isLoading ? (
              ...
              <Text style={{ color: ((!pendingVerification && isValidEmail) || (pendingVerification && isValidCode)) ? colorRoles.content.onPrimary : colorRoles.content.secondary, fontFamily: typography.bodyLarge.fontFamily, fontSize: typography.bodyLarge.fontSize, fontWeight: '700' }}>
                {!pendingVerification ? "Send Code" : "Verify & Create Passkey"}
              </Text>
            )}
          </PressableScale>
        </Animated.View>
