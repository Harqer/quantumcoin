  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colorRoles.background.primary }} edges={['bottom', 'top']}>
      <ScrollView contentContainerStyle={{ padding: spacing.xl, flexGrow: 1 }}>
        ...
        <TextInput testID="input-auto-53967b" testID="input-auto-59598a" style={{ backgroundColor: colorRoles.background.secondary, color: colorRoles.content.primary, padding: spacing.xl, borderRadius: 16, marginBottom: spacing.xl, fontSize: 32, textAlign: 'center', fontWeight: 'bold' }} placeholder="$0.00" placeholderTextColor={colorRoles.content.secondary} keyboardType="numeric" value={amount} onChangeText={setAmount} />

        <View style={{ flex: 1 }} />

        <PressableScale testID="btn-auto-89a615" testID="btn-auto-7a6f7a" onPress={handleTransfer} disabled={transferMutation.isPending} style={{ backgroundColor: transferMutation.isPending ? colorRoles.background.disabled : colorRoles.content.accentMid, paddingVertical: spacing.l, borderRadius: 999, alignItems: 'center', width: '100%' }}>
